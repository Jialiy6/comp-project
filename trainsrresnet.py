...
def main():
    ...
    # 初始化
    srresnet_model = SRResNet(large_kernel_size=large_kernel_size,  # 9
                              small_kernel_size=small_kernel_size,  # 3
                              n_channels=n_channels,  # 64
                              n_blocks=n_blocks,  # 16
                              scaling_factor=scaling_factor)  # 4
    srresnet_model.load_state_dict(torch.load('results/checkpoint_srresnet.pth'), strict=False)

    # 初始化优化器
    optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, srresnet_model.parameters()), lr=lr)
    ...
    if checkpoint is not None:
        ...
        print("现在开始第"+str(start_epoch)+"次训练")
        srresnet_model.load_state_dict(checkpoint['model'])
        optimizer.load_state_dict(checkpoint['optimizer'])

    # 加载数据
    train_dataset = SRDataset(data_folder, kind='train',
                              crop_size=crop_size,
                              scaling_factor=scaling_factor,
                              lr_type='imagenet',
                              hr_type='[-1, 1]')
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               batch_size=batch_size,  # 按照电脑性能更改
                                               shuffle=True,
                                               num_workers=workers,
                                               pin_memory=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        srresnet_model.train()  # 训练模式：允许使用批样本归一化 启用BatchNormalization和Dropout
        ...
        # 按批处理
        for i, (lr_imgs, hr_imgs) in enumerate(train_loader):  # 枚举

            # 数据迁移
            lr_imgs = lr_imgs.to(device)  # (batch_size (N), 3, 24, 24), imagenet格式
            hr_imgs = hr_imgs.to(device)  # (batch_size (N), 3, 96, 96),  [-1, 1]格式

            # 前向传播
            sr_imgs = srresnet_model(lr_imgs)
            loss = criterion(sr_imgs, hr_imgs)  # 计算损失

            # 后向传播
            optimizer.zero_grad()  # 首先进行梯度清零
            loss.backward()
            optimizer.step()  # 更新模型
        ...
        torch.save({
            'epoch': epoch,
            'model': srresnet_model.state_dict(),
